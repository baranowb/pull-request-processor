    ...
    public void eval(EvaluatorContext context, EvaluatorData data) {
        ...
        if (upstreamPullRequestData.isDefined()) {
            if (upstreamPullRequestData.isMerged()) {
                labelData.addLabelItem(new LabelItem(LabelItem.LabelContent.Upstream_merged, LabelItem.LabelAction.SET,
                        LabelItem.LabelSeverity.OK));
            } else {
                // TODO: check if this can happen
                labelData.addLabelItem(new LabelItem(LabelItem.LabelContent.Upstream_merged, LabelItem.LabelAction.REMOVE,
                        LabelItem.LabelSeverity.BAD));
            }
        } else {
            // not defined, check if it is required
            if (upstreamPullRequestData.isRequired()) {
                labelData.addLabelItem(new LabelItem(LabelItem.LabelContent.Missing_upstream_PR, LabelItem.LabelAction.REMOVE,
                        LabelItem.LabelSeverity.BAD));
            }
        }
    }
