...
public class IssueACKFlagsLabelEvaluator extends AbstractLabelEvaluator {
    ...
    protected void processAckLabels(final Attribute<IssueData> issueKey, final Attribute<LabelData> labelsKey,
        ...
        if (issueToProcess.isDefined()) {
            ...
            hasAllAcks = hasAllAcks & processFlagStatus(labelData, issueToProcess.getPmAckStatus(), LabelItem.LabelContent.Needs_pm_ack);
            hasAllAcks = hasAllAcks & processFlagStatus(labelData, issueToProcess.getDevAckStatus(), LabelItem.LabelContent.Needs_devel_ack);
            hasAllAcks = hasAllAcks  & processFlagStatus(labelData, issueToProcess.getQeAckStatus(), LabelItem.LabelContent.Needs_qe_ack);

            if (hasAllAcks) {
                LabelItem li = new LabelItem(LabelItem.LabelContent.Has_All_Acks, LabelItem.LabelAction.SET, LabelItem.LabelSeverity.OK);
                labelData.addLabelItem(li);
            } else {
                LabelItem li = new LabelItem(LabelItem.LabelContent.Has_All_Acks, LabelItem.LabelAction.REMOVE, LabelItem.LabelSeverity.BAD);
                labelData.addLabelItem(li);
            }
        }
    }
